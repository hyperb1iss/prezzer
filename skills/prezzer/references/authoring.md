...
- **Factual outline.** Every slide gets a stable id (`S1`, `S2`, …), a title, an act, and the one claim it makes. Ids survive reorders; use them in filenames, notes, and commit messages.
...
One load-bearing idea per slide is a better default than shrinking text to fit. If a slide needs three ideas, it's three beats or three slides. The audience reads the slide in two seconds and listens to you for the rest — write for that split.
