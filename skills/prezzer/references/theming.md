...
`theme.glow` — prebuilt box-shadow strings: `cyan`, `purple`, `coral`, `green`, `yellow`. Use for `textShadow`/`boxShadow` emphasis.

`theme.fonts` — `display` (Clash Display), `body` (Satoshi), `mono` (Geist Mono). See Fonts below for offline behavior.

`theme.layers` — z-index scale: `background` 0, `grid` 1, `scanlines` 2, `content` 10, `particles` 20, `navigation` 30, `overlay` 40. Slot custom layers into this scale instead of inventing z-indexes.
